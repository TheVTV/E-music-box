...
Do złożenia programatora korzystam z Arduino MEGA które znacząco pomoże mi z programowaniem pamięci, ze względu na ilość pinów cyfrowych (osobiście posiadam esp32, jednak nie dość, że posiada ono za mało pinów, to działa na 3.3V. Na całe szczęście mój znajomy pożyczył mi Arduino, aby nie bawić się z rejestrami i różnicą napięć). Suma summarum, prawie wszystkie piny z EEPROM podpinam do pinów cyfrowych Arduino. Prawie, ponieważ pin 28 wpinam do VCC, pin 14 do masy, a piny 1 i 26 (NC – no connect) zostają niepodłączone. Zwracam uwagę na 3 szczególne piny – 20, 22 i 27 – gdyż służą one do sterowania trybem pamięci. Pin 20 to zanegowany CE (Chip Enable), który po podaniu wartości 0 włącza układ. Pin 22 to zanegowany OE (Output Enable), który po podaniu wartości 0 przełącza układ w tryb do odczytu. Pin 27 to zanegowany WE (Write Enable), który po podaniu wartości 0 przełącza układ w tryb do zapisu. Oczywiście nie możemy dopuścić do sytuacji, kiedy w tym samym momencie na pinach OE i WE będzie wartość 0 (nie może być jednocześnie zapis i odczyt, mogłoby to zepsuć układ), toteż postanowiłem dołożyć rezystory „pull-up” zarówno do tych pinów, jak i do CE. Dzięki temu te piny domyślnie zawsze będą ustawione na wartość 1, a jeśli chciałbym uruchomić poszczególny tryb poprzez Arduino, wystarczy, że „ściągnę” (programowo) prąd na jego masę. Używam tego, ponieważ czas od uruchomienia płytki do ustawienia wartości na danym pinie może być na tyle duży, że zaistnieje sytuacja, w której te właśnie piny będą jednocześnie ustawione na low. 
